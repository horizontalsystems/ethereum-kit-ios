extension Geth {
    ...
    ///   - completionHandler:
    public func getTokenBalance(contractAddress: String, address: String, completionHandler: @escaping (Result<Balance>) -> Void) {
        
        let data = ERC20.ContractFunctions.balanceOf(address: address).data

        self.call(to: contractAddress, data: data.toHexString().addHexPrefix()) { result in
