...
public class RpcTransaction: ImmutableMappable {
    ...
    public required init(map: Map) throws {
        ...
        gas = try map.value("gas", using: HexIntTransform())
...
extension RpcTransaction: CustomStringConvertible {
    ...
    public var description: String {
        "[hash: \(hash.toHexString()); nonce: \(nonce); blockHash: \(blockHash?.toHexString() ?? "nil"); blockNumber: \(blockNumber.map { "\($0)" } ?? "nil"); transactionIndex: \(transactionIndex.map { "\($0)" } ?? "nil"); from: \(from.hex); to: \(to?.hex ?? "nil"); value: \(value); gasPrice: \(gasPrice); gas: \(gas); input: \(input.hex)]"
    }
